...
UART_HandleTypeDef huart1; // To ESP32 (Blynk)
UART_HandleTypeDef huart2; // To PC (Debug)
...
static void MX_USART1_UART_Init(void); // New ESP32 Port
...
int main(void) {
    ...
    while (1) {
        // 1. DHT11 (Room Temp)
        if (HAL_GetTick() - lastDHTRead >= 2000) {
            DHT11_Start();
        ...
        // 6. Data Transmission
        if (HAL_GetTick() - lastSerialSend >= 200) { // Slower interval for better Blynk processing
            // Format: ECG,BPM,BodyTemp,RoomTemp,Humidity
            sprintf(esp_msg, "%lu,%d,%d.%d,%d,%d\r\n", ecgValue, bpm, wholePart, decimalPart, Temp, Hum);

            // Send to ESP32 via UART1
            HAL_UART_Transmit(&huart1, (uint8_t*)esp_msg, strlen(esp_msg), 100);

            // Send Debug info to PC via UART2
            sprintf(msg, "DEBUG: %s", esp_msg);
            HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), 100);

            lastSerialSend = HAL_GetTick();
        }
...
/* UART1: PA9 (TX), PA10 (RX) - ESP32 Connection */
...
/* USART2: PA2 (TX), PA3 (RX) - Debug Connection */
static void MX_USART2_UART_Init(void) {
...
/* --- Rest of Peripherals and Config remains same as provided --- */
